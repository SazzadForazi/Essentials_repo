...
# =============================================================================
# Globals
DETECTOR_IP = '192.168.1.156'
DETECTOR_PORT = '5008'
RECOGNIZER_IP = '192.168.1.156'
...
def main(input):
    for n, file in enumerate(read_image(input)):
        ...
        for i, plate in enumerate(detected_plates):
            ...
            if args.recognize:
                ...
                if 0.9 <= cs <= 1.0:
                    ...
                    city_confidence = 'low'
                detected_plates[i]['text'] = "{}".format(city)
                detected_plates[i]['city_confidence'] = city_confidence
                ...
                if 0.9 <= ns <= 1.0:
                    ...
                    number_confidence = 'low'
                detected_plates[i]['number'] = "{}".format(number)
                detected_plates[i]['number_confidence'] = number_confidence

                if valid_number(resp[1][1]) and city_confidence in ('high', 'medium') and number_confidence in ('high', 'medium',):
                    detected_plates[i]['valid'] = True
                else:
                    detected_plates[i]['valid'] = False
        ...
        # Prepare output for console
        output = ''
        # for plate in detected_plates:
        #     if plate['valid']:

        output = ', '.join(["{}({}) {}({})".format(x['text'], x['city_confidence'], x['number'], x['number_confidence']) for x in detected_plates if x['valid']])

        # Print in console
        if n == 0:
            print("{:<50} {}".format('FILENAME', 'PLATES'))
            print("{:<50} {}".format('-' * 50, '-' * 15))
        print("{:<50} {}".format(filename, output))

        # if n == 0:
        #     print("{:<50} {:<10} {:13} {}".format('FILENAME', '# PLATES', 'TIME TOOK(ms)', 'PLATES'))
        #     print("{:<50} {:<10} {:13} {}".format('-' * 50, '-' * 10, '-' * 13, '-' * 15))
        # print("{:<50} {:<10} {:>13.2f} {}".format(filename, len(detected_plates), (t1 - t0) * 1000, output))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--input', type=path_validator, default='/home/tigerit/Downloads/192.168.1.223/0004')
    ...
    parser.add_argument('--ip', type=str, default='192.168.1.156',
