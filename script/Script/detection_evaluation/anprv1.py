...
def draw(filepath, plates):
    ...
    for plate in plates:
        ...
        if 'text' in plate and 'number' in plate:
            text = "{} {}".format(plate['text'], plate['number'])
        else:
...
def main(input):
    for n, file in enumerate(read_image(input)):
        ...
        if len(detected_plates) == 0:
            ...
            shutil.copyfile(src=filepath, dst=os.path.join(NOTDETECTED_DIR, filename))

        if not detected_plates:
            cv2.imwrite(os.path.join(args.outputdir, 'frames', '{}.jpg'.format(filename)), image_to_write)
        ...
        for i, plate in enumerate(detected_plates):
            ...
            if not args.detector_only:
                ...
                city = {'msg': resp[0][1]}
                cs = resp[0][2]
                detected_plates[i]['text'] = city

                number = {'msg': resp[1][1]}
                ns = resp[1][2]
                detected_plates[i]['number'] = number
...
if __name__ == '__main__':
    ...
    parser.add_argument('--detector-only', default=True, action='store_true')
    ...
    NOTDETECTED_DIR = os.path.join(OUTPUT_DIR, "not_detected")

    os.makedirs(FRAME_DIR, exist_ok=True)
    os.makedirs(TEXT_DIR, exist_ok=True)
    os.makedirs(NUMBER_DIR, exist_ok=True)
    os.makedirs(PLATE_DIR, exist_ok=True)
    os.makedirs(NOTDETECTED_DIR, exist_ok=True)
